...
- If a target is not configured, the mod may log a limited `No EXP rule` message.
- Changes to `exp_rules.json` require a full game restart.

Known issues:

- You can expect stutter/hitch on the 1st. kill
- You can expect stutter/hitch when game/code is cached
- Mod is experimental, was tested on Dodos and Boars
...
Do not install the mod into a backup or different folder. 
The active folder must be named `ue4ss`.
